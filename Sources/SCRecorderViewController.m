...
@property (nonatomic) float recordingTime; //to pass record session duration to Swift file
...
- (void)viewDidLoad {
    ...
    [newProduct setPurchased:[NSNumber numberWithInt:0]];
    NSLog(@"\n\n newproduct added in recording view");
...
//    _recorder.fastRecordMethodEnabled = YES;
    ...
    self.focusView.outsideFocusTargetImage = [UIImage imageNamed:@"focuse_"];
    self.focusView.insideFocusTargetImage = [UIImage imageNamed:@"focuse_"];
...
- (void)updateTimeRecordedLabel {
    ...
    if (_recorder.session != nil) {
        ...
        _recordingTime = CMTimeGetSeconds(_recorder.session.duration);
//        NSLog(@"recordingTime 1 : %f", _recordingTime);
    }
    
    self.timeRecordedLabel.text = [NSString stringWithFormat:@"%.2f sec", CMTimeGetSeconds(currentTime)];

    if(_recordingTime > 7.0f){
        [_recorder pause:^{
            [self saveAndShowSession:_recorder.session];
        }];
    }
